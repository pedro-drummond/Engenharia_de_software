#include "../model_implementation.h"
...
BaseModel::~BaseModel() {
    ...
    for(auto mod = models.begin(); mod < models.end(); mod++) {
        if ((*mod) == this) {
            models.erase(mod);
        }
    }
...
void BaseModel::add(Model* model) {
...
Model* Model::createModel(string name) {
    return BaseModel::createModel(name);
}

Model* BaseModel::createModel(string name) {
    Model* model = new BaseModel(name);
    models.push_back(model);
    return model;
}
System* BaseModel::createSystem(string name, double value) {
    System* system = new BaseSystem(name, value);
    add(system);
    return system;
}
